...
export async function overview(scope) {
  ...
  const allSeats = seatScopeWhere(scope);
  if (!memberWhere || !allSeats) return null;

  /* Coverage counts the organisational structure — national through ward — and
     deliberately leaves the polling unit tier out of it.

     There are 176,623 booth seats against 92,184 in every other tier combined,
     so folding them in would mean a National Coordinator's "structure coverage"
     was really "booth agent coverage" wearing a different label, and filling
     the entire national executive would move it by 0.006%. They are two
     different jobs with two different timescales, so they get two figures. */
  const seatWhere = { AND: [allSeats, { scopeType: { not: "POLLING_UNIT" } }] };
  const agentWhere = { AND: [allSeats, { scopeType: "POLLING_UNIT" }] };
  ...
  const [
    ...
    prisma.member.count({
      where: { ...memberWhere, joinedAt: { gte: thirtyDaysAgo } },
    }),
    ...
    prisma.member.count({
      where: { ...memberWhere, referredById: { not: null } },
    }),
    prisma.member.count({
      where: {
        ...memberWhere,
    ...
    prisma.seat.count({ where: { AND: [seatWhere, { status: "FILLED" }] } }),
    prisma.seat.count({ where: agentWhere }),
    prisma.seat.count({ where: { AND: [agentWhere, { status: "FILLED" }] } }),
  ]);
  ...
  return {
    ...
    /* The election-day figure: how many of the polling units in this territory
       have somebody who will actually be standing in them. */
    booths,
    agents,
    agentCoverage: booths ? Math.round((agents / booths) * 100) : 0,
  };
}
...
async function leadOf(level, id) {
  ...
  const seat = await prisma.seat.findFirst({
    where: {
      ...(level === "zone"
        ? { zoneId: id, role: { code: { startsWith: "ZC_" } } }
        : {}),
...
export async function ownSeats(scope) {
  ...
  const seats = await prisma.seat.findMany({
    ...
    select: {
      ...
      status: true,
      role: {
        select: { code: true, title: true, isAdmin: true, tierRank: true },
      },
      appointments: {
        where: { status: "ACTIVE" },
        take: 1,
