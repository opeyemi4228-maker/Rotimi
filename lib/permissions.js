...
export const TIER_RANK = {
  ...
  LGA: 4,
  WARD: 5,
  // The sixth tier. A Polling Unit Coordinator administers nobody — they are
  // the bottom of the Descendant Rule, and every check that compares tier ranks
  // now has a floor below the ward.
  POLLING_UNIT: 6,
};
...
export async function resolveScope(memberId) {
    ...
    include: {
      seat: {
        include: {
          ...
          pollingUnit: { include: { ward: { include: { lga: { include: { state: true } } } } } },
        },
      },
    },
  });
  ...
  const scope = {
    ...
    lgaId: seat.lgaId,
    /* A polling unit seat carries no wardId of its own, but every scope check
       above it needs one — so the unit's ward is resolved here and the rest of
       the file never has to special-case the sixth tier. */
    wardId: seat.wardId ?? seat.pollingUnit?.wardId ?? null,
    pollingUnitId: seat.pollingUnitId,
...
function scopeLabel(seat) {
  if (seat.pollingUnit)
    return `${seat.pollingUnit.name}, ${seat.pollingUnit.ward.name} Ward, ${seat.pollingUnit.ward.lga.name}`;
...
function scopeTier(seat) {
  if (seat.pollingUnit) return "Polling unit";
...
function scopeUnit(seat) {
  if (seat.pollingUnit) return seat.pollingUnit.name;
...
export function memberScopeWhere(scope) {
  ...
  switch (scope.scopeType) {
    ...
    case "POLLING_UNIT":
      /* Members are registered to a polling unit optionally, so this is the one
         scope where the directory can legitimately be empty. It is still the
         right filter: a booth agent has no business reading the ward roll. */
      return { pollingUnitId: scope.pollingUnitId };
    default:
...
/**
 * The same filter, expressed as the ids `lib/elections.js` wants.
 *
 * Results carry a denormalised state, LGA and ward written by the server from
 * the polling unit, so the filter is a direct column match at every level and
 * needs no joins.
 *
 * Returns `{ level, parentId, ...ids }` — `level` is the depth the reader
 * stands at, so the page knows what the units *below* them are called: a State
 * Coordinator's children are LGAs, a Ward Coordinator's are booths.
 */
export function resultScope(scope) {
  if (!scope) return null;

  /* Nationwide readers get the federation, which is the absence of a filter
     rather than an empty one — the same distinction resultsWhere() makes. */
  if (scope.isSuperAdmin || scope.readsNationwide || scope.scopeType === "NATION") {
    return { level: "nation", parentId: null, label: "Federation" };
  }

  switch (scope.scopeType) {
    case "ZONE":
      /* A zone has no column on a result. It is a set of states, and the one
         place in this file where the filter cannot be a single id — so the
         caller is told to fan out rather than being handed a wrong answer. */
      return { level: "zone", parentId: scope.zoneId, zoneId: scope.zoneId };
    case "STATE":
      return { level: "state", parentId: scope.stateId, stateId: scope.stateId };
    case "LGA":
      return { level: "lga", parentId: scope.lgaId, lgaId: scope.lgaId };
    case "WARD":
      return { level: "ward", parentId: scope.wardId, wardId: scope.wardId };
    case "POLLING_UNIT":
      return {
        level: "pollingUnit",
        parentId: scope.pollingUnitId,
    ...
    case "ZONE":
      return {
        OR: [
          ...
          { pollingUnit: { ward: { lga: { state: { zoneId: scope.zoneId } } } } },
        ],
      };
    case "STATE":
      return {
        OR: [
          ...
          { pollingUnit: { ward: { lga: { stateId: scope.stateId } } } },
        ],
      };
    case "LGA":
      return {
        OR: [
          { lgaId: scope.lgaId },
          { ward: { lgaId: scope.lgaId } },
          { pollingUnit: { ward: { lgaId: scope.lgaId } } },
        ],
      };
    case "WARD":
      /* A Ward Coordinator appoints the polling unit agents beneath them, so
         their seat scope has to include those seats as well as their own. */
      return { OR: [{ wardId: scope.wardId }, { pollingUnit: { wardId: scope.wardId } }] };
    case "POLLING_UNIT":
      return { pollingUnitId: scope.pollingUnitId };
    default:
      return null;
...
/**
 * Is this actor the office that appoints to this seat?
 *
 * ── WHY THIS IS NARROWER THAN canAdminister() ──────────────────────────────
 * canAdminister() answers "may this actor act on this seat at all" — strictly
 * lower tier, inside the territory. That is the right rule for reading and for
 * removal, and it is too loose for appointment: under it a National Coordinator
 * could reach past four tiers and put somebody into a ward directly, which is
 * not how the movement is meant to work. The structure appoints downwards one
 * step at a time — national to state, state to LGA, LGA to ward, ward to booth
 * — so that every officer is answerable to the person who put them there.
 *
 * The chain is not invented here. Every RoleDefinition carries `approverRole`,
 * seeded from the plan, and this function reads it. One place to change if the
 * movement ever reorganises.
 *
 * ── THE TWO SPECIAL CASES ──────────────────────────────────────────────────
 * `ZONAL_COORD_OF_ZONE` is a placeholder rather than a role code: the approver
 * is whichever zonal seat governs the zone the vacancy is in, which cannot be
 * written as a fixed code. It resolves to "any ZONAL-tier admin whose territory
 * contains this seat".
 *
 * And the Super Admin may act anywhere, per §6.9 — the same exception every
 * other check in this file makes.
 * ───────────────────────────────────────────────────────────────────────────
 */
export function isApproverFor(scope, seat) {
  if (!scope || !seat?.role) return false;
  if (scope.isSuperAdmin) return true;

  /* Everything canAdminister insists on still applies: a functional director
     appoints nobody, and nobody reaches outside their own territory. */
  if (!canAdminister(scope, seat)) return false;

  const approver = seat.role.approverRole;
  if (!approver) return false;

  if (approver === "ZONAL_COORD_OF_ZONE") {
    return scope.tier === "ZONAL" && scope.isAdmin && !scope.isFunctional;
  }

  return scope.roleCode === approver;
}
...
export function scopeContains(scope, seat) {
  ...
  switch (scope.scopeType) {
    ...
    case "WARD":
      return seat.wardId === scope.wardId || seat.pollingUnit?.wardId === scope.wardId;
    case "POLLING_UNIT":
      // The bottom of the structure. A booth agent appoints nobody.
      return false;
    default:
      return false;
  }
...
function seatZoneId(seat) {
  return (
    ...
    seat.pollingUnit?.ward?.lga?.state?.zoneId ??
    null
  );
}
...
function seatLgaId(seat) {
  return seat.lgaId ?? seat.ward?.lgaId ?? seat.pollingUnit?.ward?.lgaId ?? null;
}
...
export function can(scope, capability) {
  ...
  switch (capability) {
    ...
    /* Speaking to a territory in the movement's name, by SMS to every member
       in it. Three conditions, and the last one is the one that is easy to
       forget: PU_AGENT is flagged isAdmin so that a booth agent can reach the
       election form, but a Polling Unit Coordinator administers nobody and has
       no territory to address — texting the handful of members who happen to
       have named that booth is not a thing the structure gives them. Ward is
       the floor. */
    case "broadcast":
      return scope.isAdmin && !scope.isFunctional && scope.tierRank <= TIER_RANK.WARD;
    case "publishContent":
