...
const LEVELS = {
  ...
  /* A zone has no column on a result — it is a set of states — so it groups by
     the same column the nation does and narrows the rows instead. It is the one
     level whose filter is a list rather than an id. */
  zone: { column: "stateId", unit: "state" },
 ...
 * every caller below.
 */
export function resultsWhere({ electionId, stateId, stateIds, lgaId, wardId, pollingUnitId }) {
  ...
  /* A zone. An empty list is a real answer — a zone with no states in it reads
     nothing — so it must not fall through to the unfiltered federation. */
  else if (stateIds) where.stateId = { in: stateIds };
  return where;
}
 ...
 */
export async function breakdown({ electionId, level, parentId, stateIds }) {
    ...
    prisma.$queryRawUnsafe(
      `SELECT r."${childColumn}" AS unit, v."partyId" AS party, SUM(v.votes)::int AS votes
        ...
        WHERE r."electionId" = $1
          ...
          ${level === "zone" ? `AND r."stateId" = ANY($2)` : parentId ? `AND r."${LEVELS[level].column}" = $2` : ""}
        GROUP BY 1, 2`,
      ...(level === "zone" ? [electionId, stateIds ?? []] : parentId ? [electionId, parentId] : [electionId])
    ),
 ...
 */
export async function reporting({ electionId, stateId, stateIds, lgaId, wardId, pollingUnitId }) {
  const where = resultsWhere({ electionId, stateId, stateIds, lgaId, wardId, pollingUnitId });

  const boothWhere = pollingUnitId
    ? { id: pollingUnitId }
    : wardId
      ? { wardId }
      : lgaId
